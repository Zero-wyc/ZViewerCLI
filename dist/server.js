...
async function createLocalProxy(options, onConnect) {
    const { port } = options;
    const app = (0, express_1.default)();
    const server = (0, http_1.createServer)(app);
    const state = {
        config: options.serverUrl && options.roomId
            ? {
    ...
    if (state.config?.cookie) {
        (0, setup_1.validateCookie)(state.config.cookie).then((validation) => {
    ...
    app.post("/api/config", async (req, res) => {
        ...
        const trimmedCookie = typeof cookie === "string" ? cookie.trim() : (state.config?.cookie ?? "");
        state.config = {
        ...
        if (trimmedCookie) {
            const validation = await (0, setup_1.validateCookie)(trimmedCookie);
            state.cookieValid = validation.valid;
            if (validation.valid) {
                state.userInfo = {
                    name: validation.name,
                ...
                await (0, config_1.savePersistedConfig)({
                    cookie: trimmedCookie,
                    userInfo: state.userInfo,
    ...
    app.get("/api/qr", async (_req, res) => {
        try {
            currentQrSession = await (0, setup_1.generateQrCode)();
            res.json({
                success: true,
    ...
    app.get("/api/qr/poll", async (req, res) => {
        ...
        try {
            const result = await (0, setup_1.pollQrStatus)(key);
            if (result.loggedIn && result.cookie && state.config) {
                state.config.cookie = result.cookie;
                const validation = await (0, setup_1.validateCookie)(result.cookie);
                state.cookieValid = validation.valid;
                if (validation.valid) {
                    state.userInfo = {
                        name: validation.name,
    ...
    app.get("/resolve", async (req, res) => {
        ...
        try {
            ...
            if (qn)
                resolveUrl.searchParams.set("qn", qn);
            resolveUrl.searchParams.set("preferMp4", String(preferMp4));
            const response = await (0, node_fetch_1.default)(resolveUrl.toString(), {
                headers: {
                    Cookie: cookie,
