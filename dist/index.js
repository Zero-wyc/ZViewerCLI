...
program
    ...
    const agent = await (0, agent_1.startCliAgent)({
...
program
    ...
    const setupUrl = `${agent.proxyUrl}?server=${encodeURIComponent(options.server || '')}&room=${encodeURIComponent(options.room || '')}`;
    if (options.open) {
...
function openBrowser(url) {
    const cmd = (0, node_os_1.platform)() === 'win32'
        ? `start "" "${url}"`
